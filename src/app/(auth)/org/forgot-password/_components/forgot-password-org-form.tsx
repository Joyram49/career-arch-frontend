...
// ── Component ────────────────────────────────────────────────────────
export function ForgotPasswordOrgForm(): React.JSX.Element {
    ...
    >
      ...
      <motion.div variants={itemVariants}>
        <Link
          ...
          aria-label="Back to employer login"
        >
      ...
      <motion.div variants={itemVariants} className="space-y-4">
        ...
        <div className="space-y-2 text-center lg:text-left">
          ...
          <p className="text-[15px] leading-relaxed text-muted-foreground">
            Enter your work email and we&apos;ll send a secure reset link to restore your employer
            account access.
          </p>
        </div>
      </motion.div>
      ...
      >
        <form
          ...
          className="space-y-4"
          noValidate
          aria-label="Forgot employer password form"
        >
          <FieldGroup>
            {/* Work Email */}
            <Controller
      ...
      >
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">
          Your employer account security is our priority. The reset link is valid for 1 hour and can
          only be used once.
        </p>
      </motion.div>
      ...
      >
        {['Encrypted', '1-Hour Expiry', 'Single-Use'].map((badge, i, arr) => (
          <span key={badge} className="flex items-center gap-2">
            <span className="text-[10px] font-bold tracking-widest text-muted-foreground/50 uppercase">
              {badge}
            </span>
            {i < arr.length - 1 && <span className="size-1 rounded-full bg-muted-foreground/30" />}
          </span>
        ))}
      </motion.div>
    </motion.div>
