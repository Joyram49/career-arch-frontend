...
function BlueprintLines(): React.JSX.Element {
  const verticals = [80, 190, 290, 390, 480];
  const horizontals = [100, 200, 320, 440, 560, 650];
    ...
    <aside className="relative hidden h-full w-full flex-col overflow-hidden px-10 py-10 lg:flex xl:px-14 xl:py-12">
      ...
      <div className="relative z-10 flex flex-1 flex-col justify-center">
        ...
        <p className="mt-6 max-w-sm text-[15px] leading-relaxed text-slate-400">
          Your employer account is protected by enterprise-grade security. Reset your password to
      ...
      {/* Bottom avatar trust section */}
      <div className="relative z-10">
        <div className="mb-5 h-px w-full bg-white/10" />
        <div className="flex items-center gap-4">
          <div className="flex -space-x-3">
            <Avatar initials="HR" colorClass="bg-brand-sky" />
            <Avatar initials="CEO" colorClass="bg-brand-emerald" />
            <Avatar initials="IT" colorClass="bg-brand-amber" />
          </div>
          <p className="text-sm leading-snug text-slate-400">
            Trusted by <span className="font-bold text-brand-sky">18,000+</span> organizations
          </p>
        </div>
      </div>
    </aside>
