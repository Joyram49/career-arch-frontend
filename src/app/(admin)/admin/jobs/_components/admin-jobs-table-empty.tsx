export function JobsEmptyState({ hasFilters }: { hasFilters: boolean }): React.JSX.Element {
  return (
    <div className="flex min-h-80 flex-col items-center justify-center px-5 text-center">
      ...
      <h3 className="text-sm font-semibold text-foreground">
        {hasFilters ? 'No jobs found' : 'No jobs yet'}
      </h3>
      <p className="mt-1 max-w-sm text-xs text-muted-foreground">
        {hasFilters
          ? 'Try adjusting your search or filters to find jobs.'
          : 'There are currently no jobs on the platform.'}
      </p>
    </div>
  );
}
